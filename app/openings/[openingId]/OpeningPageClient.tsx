...
export default function OpeningPageClient({ openingId }: { openingId: string }) {
  ...
  const studied = opening ? progress.openingsStudied.includes(opening.id) : false;
  ...
  const handleNext = useCallback(() => {
    setCurrentMoveIdx(i => {
      ...
      if (opening && next === displayLine.length - 1 && !studied) studyOpening(opening.id);
      return next;
    });
  }, [displayLine.length, opening, studied, studyOpening]);
